...
inputFile=$4 #only directory
outputDir=$5 #output directory name
sampleType=$6 #sample short name
mcType=$7 ##mc campaign
sysType=$8 ##systematice type

echo "in is" $inputFile
echo "outDir is " $outputDir
echo "sample is " $sampleType
echo "sys is " $sysType
echo "mc campaign is " $mcType
...
python makeAllHists.py $inputFile $outputDir $sampleType $mcType $sysType
