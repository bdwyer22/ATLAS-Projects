...
BATCH_ROOT_DIR="/xdata/${MYUSERNAME}/batch"
...
### here we passed "any" since we are happy to submit to any node, and not a specific one
if [ "$3" != "any" ]; then 
   SHORT_NODE_NAME="$3"
...
inputdir=$4 #only directory
outputname=$5 #output directory name
sampleType=$6 #mc16a/d/e data yaer
filename=$7 ##specific file name 
sysType=$8 ##systematice type

echo "in from" $filename
echo "out is " $outputname
echo "mc campaign is " $sampleType
echo "sample is " $sysType
...
if [ ! -f ${PROJECT_CODE_DIR}/project_archive.tgz ]; then 
 ...
 tar -czf ${PROJECT_CODE_DIR}/project_archive.tgz  --directory  $PROJECT_CODE_DIR ./source > /dev/null 2>&1
else
...
### this is hard-coded for now! you might want to change it
asetup AnalysisBase,21.2.162,here
cd ..
### this is for cmake compilation
mkdir build
cd build
cmake ../source 
cmake --build . 
source x86_64-slc7-gcc8-opt/setup.sh
mkdir $JOBSCRATCH/runLocal
cd    $JOBSCRATCH/runLocal


echo "bls doing ls in run directory"
ls -ltr
#
...
echo "BLS input dir = " $PROJECT_SCRATCH_DATADIR/
echo "bls outputname = " $outputname
echo "bls in directory" ${PWD}



yyHFSkim_RunLocal $filename $sampleType $sysType $outputname


mv $outputname/hist-sample.root ./$outputname.root
#
echo "Done and ls is"
ls -l
echo "BLS output"
...
rm -rf ./data_yyHF_skimMxAODs*
rm -rf ./data_yyHF_combinedCR*
echo "checking ls again after removal"
ls -l
#
EOF_USER_SCRIPT
#
